...
export async function registerForTournament(data: z.infer<typeof baseRegistrationSchema>) {
  ...
  if (tournament.maxParticipants && currentRegistrations >= tournament.maxParticipants) {
  ...
  for (const player of players) {
    for (const field of tournament.fields) {
      ...
      if (field.required && (!value || value.trim() === "")) {
        return { message: `Missing required field: ${field.label} for player ${player.nickname}` };
      }
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const player of players) {
        ...
        // Create PlayerData
        const dataEntries = Object.entries(player.data).map(([fieldId, value]) => ({
