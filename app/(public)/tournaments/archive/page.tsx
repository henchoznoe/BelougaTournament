      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {tournaments.length > 0 ? (
          tournaments.map((tournament) => (
            <Card key={tournament.id} className="border-zinc-800 bg-zinc-900/50 opacity-80 hover:opacity-100 transition-opacity">
              <CardHeader>
                <CardTitle className="text-xl text-zinc-200">{tournament.title}</CardTitle>
              ...
              <CardFooter>
                <Button asChild variant="secondary" className="w-full bg-zinc-800 text-zinc-300 hover:bg-zinc-700">
                  <Link href={`/tournaments/${tournament.slug}`}>View Results</Link>
                </Button>
              </CardFooter>
            </Card>
          ))
