...
export default async function TournamentPage({
  ...
  const now = new Date();
  const isRegistrationOpen = now >= tournament.registrationOpen && now <= tournament.registrationClose;

  return (
    <div className="container mx-auto max-w-5xl px-4 py-12">
      <div className="mb-8 space-y-4">
        <h1 className="text-4xl font-extrabold text-white sm:text-5xl">{tournament.title}</h1>
      ...
      <div className="grid gap-12 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-8">
          <div className="prose prose-invert max-w-none rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
            <h3 className="text-xl font-bold text-white">About this Tournament</h3>
            <p className="whitespace-pre-wrap text-zinc-300">{tournament.description}</p>
          </div>
            ...
            {isRegistrationOpen ? (
              <div className="space-y-4">
                <p className="text-sm text-zinc-400">
                  Registration is open until {new Date(tournament.registrationClose).toLocaleDateString()}.
                </p>
              ...
              <div className="rounded-lg bg-red-900/20 p-4 text-center text-red-200 border border-red-900/50">
                <p className="font-semibold">Registration is currently closed.</p>
